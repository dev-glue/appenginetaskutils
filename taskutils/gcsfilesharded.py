...
def futuregcsfileshardedpagemap(pagemapf=None, gcspath=None, pagesize=100, onsuccessf=None, onfailuref=None,
    ...
    def MapOverRange(futurekey, startbyte, endbyte, weight, **kwargs):
        ...
        try:
            ...
            if pagemapf:
                ...
                future(pagemapf, parentkey=futurekey, onallchildsuccessf=lonallchildsuccessf, weight=len(page),
                       **taskkwargs)(page)
            else:
            ...
            if ranges:
                ...
                for arange in ranges:
                    ...
                    lonallchildsuccessf = GenerateOnAllChildSuccess(futurekey,
                                                                    linitialresult if pagemapf else len(page),
                                                                    loncombineresultsf)

                    future(MapOverRange, parentkey=futurekey, onallchildsuccessf=lonallchildsuccessf, weight=newweight,
                           **taskkwargs)(arange[0], arange[1], weight=newweight)
    ...
    return future(MapOverRange, onsuccessf=onsuccessf, onfailuref=onfailuref, onprogressf=onprogressf,
                  onallchildsuccessf=onallchildsuccessf, parentkey=parentkey, weight=weight, **taskkwargscopy)(0,
                                                                                                               filesizebytes,
                                                                                                               weight)
...
def futuregcsfileshardedmap(mapf=None, gcspath=None, pagesize=100, onsuccessf=None, onfailuref=None, onprogressf=None,
    ...
    return futuregcsfileshardedpagemap(pageMapF, gcspath, pagesize, onsuccessf=onsuccessf, onfailuref=onfailuref,
...
def futuregcscompose(gcsbucket=None, gcssourceprefix=None, gcstargetprefix=None, gcstargetfilename="output.txt",
                     onsuccessf=None, onfailuref=None, onprogressf=None, initialresult=None, oncombineresultsf=None,
                     weight=None, parentkey=None, **taskkwargs):
        ...
        try:
            ...
            if numfiles > 32:
                ...
                for r in ranges:
                    ...
                    future(GCSCombineToTarget, futurename=futurename, onallchildsuccessf=onallchildsuccessf,
                           parentkey=futurekey, weight=r[1] - r[0], **taskkwargs)(r[0], r[1], False)
                raise FutureReadyForResult()
            else:
                lblobs = list(listbucket(gcsbucket, gcssourceprefix))[startindex:finishindex]
                lfilename = "%s/%s" % (
                    gcstargetprefix, gcstargetfilename if istop else "composed-%s-%s" % (startindex, finishindex))
    ...
    return future(GCSCombineToTarget, futurename=futurename, onsuccessf=onsuccessf, onfailuref=onfailuref,
                  onprogressf=onprogressf, parentkey=parentkey, weight=numgcsfiles, **taskkwargs)(0, numgcsfiles, True)
...
def hwalk(afile, pagesizeinlines, numranges, startbytes, endbytes):
    ...
    if rangesstartpos < endbytes:
        ...
        ranges = [[int(rangesstartpos + rangeindex * rangesize), int(rangesstartpos + (rangeindex + 1) * rangesize)] for
                  rangeindex in range(numranges)]
        if ranges:
